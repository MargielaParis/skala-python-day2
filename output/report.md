...
- 소요 시간(단일 참고 측정, 벤치마크 아님)
  - train: Pandas 0.034초 vs Polars 0.020초
    (32,561행 x 15컬럼)
  - test: Pandas 0.018초 vs Polars 0.013초
    (16,281행 x 15컬럼)
...
- **`capital-gain`은 상한 처리된 값이다.** 학습 데이터에서 0인 비율이
  ...
  "그 이상"을 뜻하는 표기이므로, 금액과 상한 도달을 분리하려고 `capital-gain-capped` 지시변수를 따로 넣었다.
...
- 피처: 수치형 5개 + 지시변수 1개 + 범주형 7개(sex·race 포함), 원핫 인코딩 후 81개
  - 수치형: age, education-num, capital-gain, capital-loss, hours-per-week
  - 지시변수: capital-gain-capped
...
|---|---:|
| 정확도 (accuracy) | 0.8474 |
| 정밀도 (precision) | 0.7291 |
| 재현율 (recall) | 0.6030 |
| F1 | 0.6601 |
| ROC-AUC | 0.9027 |
| PR-AUC (average precision) | 0.7650 |
...
- 실제 고소득 3,700명 중
  1,469명을 저소득으로 잘못 예측했다 (재현율 0.6030).
- 실제 저소득 11,355명 중
  829명을 고소득으로 잘못 예측했다.
...
|---|---:|---:|---:|---:|---:|---:|
| 기본값 0.500 | 0.8474 | 0.7291 | 0.6030 | 0.6601 | 1,469 | 829 |
| 학습셋 F1 최적 0.341 | 0.8333 | 0.6367 | 0.7497 | 0.6886 | 926 | 1,583 |
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 결측 행 삭제(dropna) | 30,139 | 15,055 | 0.8474 | 0.7291 | 0.6030 | 0.6601 | 0.9027 | 0.7650 |
| 범주형 결측을 Unknown으로 보존 | 32,537 | 16,276 | 0.8522 | 0.7274 | 0.5993 | 0.6572 | 0.9049 | 0.7619 |
...
- 지시변수(`capital-gain-capped`)는 스케일링하지 않아 계수를 **0에서 1로 바뀔 때**로 그대로 읽는다.
...
|:---------------------------------|------:|-------:|:----------------|:-------------------------------------|:------------------|
| capital-gain                     | 2.319 | 10.162 | [8.728, 11.832] | 1 표준편차 증가                            | 30,139            |
| relationship_Wife                | 1.289 |  3.629 | [2.961, 4.447]  | 기준 relationship=Husband              | 1,406 / 기준 12,457 |
| native-country_Cambodia          | 0.857 |  2.356 | [0.841, 6.599]  | 기준 native-country=United-States      | 18 / 기준 27,487    |
| native-country_Italy             | 0.753 |  2.124 | [1.147, 3.932]  | 기준 native-country=United-States      | 68 / 기준 27,487    |
| education-num                    | 0.725 |  2.064 | [1.967, 2.165]  | 1 표준편차 증가                            | 30,139            |
| marital-status_Married-AF-spouse | 0.591 |  1.806 | [0.739, 4.415]  | 기준 marital-status=Married-civ-spouse | 21 / 기준 14,059    |
| workclass_Federal-gov            | 0.485 |  1.624 | [1.355, 1.948]  | 기준 workclass=Private                 | 943 / 기준 22,264   |
| hours-per-week                   | 0.352 |  1.423 | [1.367, 1.480]  | 1 표준편차 증가                            | 30,139            |

### 4-6. 고소득 오즈와 음의 연관이 큰 항목 (하위 8개)

|                              |     계수 |   오즈비 | 오즈비 95% 구간     | 비교 기준                                | 학습 표본             |
|:-----------------------------|-------:|------:|:---------------|:-------------------------------------|:------------------|
| marital-status_Never-married | -1.855 | 0.157 | [0.101, 0.243] | 기준 marital-status=Married-civ-spouse | 9,711 / 기준 14,059 |
| occupation_Priv-house-serv   | -1.786 | 0.168 | [0.051, 0.552] | 기준 occupation=Prof-specialty         | 141 / 기준 4,034    |
| occupation_Farming-fishing   | -1.495 | 0.224 | [0.173, 0.290] | 기준 occupation=Prof-specialty         | 987 / 기준 4,034    |
| native-country_Columbia      | -1.467 | 0.231 | [0.079, 0.671] | 기준 native-country=United-States      | 56 / 기준 27,487    |
| marital-status_Separated     | -1.445 | 0.236 | [0.141, 0.394] | 기준 marital-status=Married-civ-spouse | 939 / 기준 14,059   |
| marital-status_Divorced      | -1.382 | 0.251 | [0.161, 0.393] | 기준 marital-status=Married-civ-spouse | 4,212 / 기준 14,059 |
| relationship_Own-child       | -1.371 | 0.254 | [0.158, 0.407] | 기준 relationship=Husband              | 4,462 / 기준 12,457 |
| occupation_Other-service     | -1.345 | 0.26  | [0.209, 0.325] | 기준 occupation=Prof-specialty         | 3,209 / 기준 4,034  |

### 4-7. sex·race 계수

|                         |     계수 |   오즈비 | 오즈비 95% 구간     | 비교 기준         | 학습 표본             |
|:------------------------|-------:|------:|:---------------|:--------------|:------------------|
| race_Asian-Pac-Islander |  0.084 | 1.088 | [0.801, 1.477] | 기준 race=White | 894 / 기준 25,912   |
| race_Black              | -0.111 | 0.895 | [0.769, 1.041] | 기준 race=White | 2,816 / 기준 25,912 |
| race_Other              | -0.521 | 0.594 | [0.338, 1.043] | 기준 race=White | 231 / 기준 25,912   |
| race_Amer-Indian-Eskimo | -0.573 | 0.564 | [0.366, 0.867] | 기준 race=White | 286 / 기준 25,912   |
| sex_Female              | -0.846 | 0.429 | [0.367, 0.502] | 기준 sex=Male   | 9,773 / 기준 20,366 |
...
|---|---:|---|
| sex_Female | 0.836 | [0.753, 0.928] |

- 기준 범주: sex=Male
- 이 모델의 정확도 0.8464 / F1 0.6560
...
- 계수 절대값이 가장 큰 항은 `capital-gain`이며 계수 +2.319, 오즈비 10.162 (95% 구간 [8.728, 11.832])다. 범주형은 기준 범주 대비, 수치형은 1 표준편차 증가 기준이다.
- 학력·직업·근로시간과 가구 내 역할(relationship)까지 고정했을 때 Female는 기준(Male) 대비 오즈 0.429배(95% 구간 [0.367, 0.502])로 추정됐다. relationship은 성별과 거의 겹치므로 이 값은 성별 전체 격차가 아니다.
- `relationship`를 빼고 다시 학습하면 sex_Female의 오즈비가 0.836(95% 구간 [0.753, 0.928])가 된다 (기준 sex=Male, 정확도 0.8464).
- 평가 성능은 정확도 0.8474, 정밀도 0.7291, 재현율 0.6030, F1 0.6601, ROC-AUC 0.9027, PR-AUC 0.7650다. 실제 고소득 3,700명 중 1,469명을 저소득으로 놓쳤다.
- 임계값을 학습셋 F1 최적값 0.341으로 바꾸면 재현율이 0.6030에서 0.7497로, 놓친 고소득이 1,469명에서 926명으로 바뀐다.
- 결측 처리 방식은 F1 기준 '결측 행 삭제(dropna)'가 0.6601로 가장 높았다.
