...
생성 일시: 2026-08-09 19:43 / 작성자: 박기연 (판교 7반)
...
|---|---|---:|---:|---|
| train | 32,561 x 15 | 0.032초 | 0.017초 | 정규화 후 컬럼 순서·shape·null mask·의미상 dtype·셀 값 불일치 0 |
| test | 16,281 x 15 | 0.017초 | 0.019초 | 정규화 후 컬럼 순서·shape·null mask·의미상 dtype·셀 값 불일치 0 |

소요 시간은 단일 참고 측정이고 벤치마크가 아니다.

### 1-2. 결측·중복 처리

적용한 전략은 결측 행 삭제(dropna)이다. `adult.test`는 첫 줄이 주석(`|1x3 Cross validator`)이고 라벨에
마침표(`>50K.`)가 붙어 있어 로딩 단계에서 주석 제외와 라벨 표기 통일을 처리했다.

| 구분 | 원본 | 결측 처리 후 | 중복 제거 후 | 결측 컬럼 |
|---|---:|---:|---:|---|
| train | 32,561 | 30,162 | 30,139 | workclass(1836건), occupation(1843건), native-country(583건) |
| test | 16,281 | 15,060 | 15,055 | workclass(963건), occupation(966건), native-country(274건) |
...
결측 제거 직후 고소득(>50K) 비율은 train이 0.2408에서 0.2489로,
test가 0.2362에서 0.2457로 이동한다(중복 제거 전 기준).

### 1-4. 결과를 읽을 때의 데이터 한계

평가셋에도 중복 제거를 적용했다. test는 15,060행에서 15,055행이 됐다.
공식 평가셋을 그대로 쓰지 않았으므로 아래 지표를 다른 참가자와 비교할 때는 같은 정제 절차를 썼는지
확인해야 한다.

train과 test에 완전히 같은 행이 19건 있다. 공식 분할이지만 이만큼은 평가가 낙관적으로
치우칠 수 있고, 이번 파이프라인은 이 행들을 제거하지 않았다.

`capital-gain`은 상한 처리된 값이다. 학습 데이터의 91.6%가 0이고, 148행이 상한값 99,999이며 그중 고소득 비율은 100.0%다. 상한값은 실제 금액이 아니라 "그 이상"을 뜻하는
...
해당 행은 전체 15,055행 중 0행이다.

`relationship`은 `sex`와 거의 겹친다(성별 편중도 Husband 100.0%, Wife 99.9%). `relationship`을 함께 통제한 상태의
...
chi2=1414.873, 자유도=1, n=30,139, p=1.232e-309. 판정: 성별과 소득은 독립이 아니다 (H0 기각).

Cramer's V는 0.217로, 표본 크기와 무관한 연관 강도를 나타낸다. 성별 고소득 비율은 Female 11.4%, Male 31.4%다.
최소 기대빈도가 2,434로 충분히 커서 Yates 연속성 보정은 쓰지 않았다. 보정은 기대빈도가 작은
...
평균은 >50K가 45.7시간, <=50K가 39.4시간으로 차이는 +6.36시간이다.
95% 신뢰구간은 [6.07, 6.64], t=43.170, p=1e-308 미만 (float64 표현 한계). 판정: 통계적으로 유의미한 차이 있음 (H0 기각).

Cohen's d는 0.545(중간)다. 표본이 크면 작은 차이도 유의해지므로 유의성과 함께
효과크기를 본다. 검정은 등분산을 가정하지 않는 Welch지만 효과크기는 관례대로 pooled 표준편차를 쓴다.

### 3-3. 기술통계와 상관

education-num과 hours-per-week의 상관계수는 0.153이다.
...
Pipeline이다. 피처는 수치형 5개 + 범주형 7개(sex·race 포함), 원핫 인코딩 후 80개다.
...
| 제외 | fnlwgt, education (fnlwgt는 표본 가중치, education은 education-num과 중복) |
| 학습 / 평가 | 30,139건 / 15,055건 |
| 고소득 비율 | train 0.249 / test 0.246 |
| 모델 파일 | `output/income_pipeline.pkl` (joblib, 재로딩 검증 완료) |
...
0.246이므로 아무 정보 없이 찍는 분류기의 PR-AUC 기준선이 그 값이다. 정확도만으로는
...
실제 고소득 3,700명 중 1,462명을 저소득으로 잘못 예측했고(재현율 0.6049), 실제 저소득 11,355명 중 833명을 고소득으로 잘못 예측했다.

### 4-2. 분류 임계값

기본 임계값 0.5는 자의적이다. F1을 최대화하는 임계값을 학습셋 5-폴드 교차검증의
...
test 15,055행으로 고정했다. 전략별로 평가셋을 따로 쓰면 행 수와 사례 구성이 달라져 지표를
나란히 비교할 수 없다.
...
`exp(beta)`다. 기준은 변수마다 학습 표본이 가장 많은 범주(동수면 사전순 앞)로 정한다. 표본이 극히 적은 범주가 기준이 되면
...
### 4-8. 민감도 분석: `relationship` 제외

`relationship` 변수는 `sex`와 거의 겹치므로(1-4절), 성별 계수가 통제 방식에 얼마나
...
기준 범주는 sex=Male, 이 모델의 정확도는 0.8465, F1은 0.6563.
...
- 고소득 그룹의 주당 근로시간은 저소득 그룹보다 +6.4시간 차이가 나고, 95% 신뢰구간은 [6.07, 6.64], Cohen's d는 0.545(중간). 통계적으로 유의미한 차이가 있다.
- 계수 절대값이 가장 큰 항은 `capital-gain`이며 계수 +2.324, 오즈비 10.222 (95% 구간 [8.776, 11.905])다. 범주형은 기준 범주 대비, 수치형은 1 표준편차 증가 기준이다.
- 학력·직업·근로시간과 가구 내 역할(relationship)까지 고정했을 때 Female의 오즈는 기준(Male) 대비 0.429배(95% 구간 [0.367, 0.501])로 추정됐다. relationship은 성별과 거의 겹치므로 이 값은 성별 전체 격차가 아니다.
- `relationship` 변수를 빼고 다시 학습하면 계수가 이렇게 바뀐다 — sex_Female 오즈비 0.835(95% 구간 [0.752, 0.927]). 기준은 sex=Male, 정확도는 0.8465.
- 평가 성능은 정확도 0.8476, 정밀도 0.7288, 재현율 0.6049, F1 0.6611, ROC-AUC 0.9027, PR-AUC 0.7651. 실제 고소득 3,700명 중 1,462명을 저소득으로 놓쳤다.
- 임계값을 교차검증 F1 최적값 0.304에 맞추면 재현율이 0.6049에서 0.7786까지 오르고, 놓친 고소득이 1,462명에서 819명으로 줄어든다.
