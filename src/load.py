...
def compare_frames(df_pd, df_pl):
...
def load_compare(file_path):
    ...
    try:
        ...
        raise SystemExit(f"[오류] 데이터 파일을 찾을 수 없습니다: {file_path}") from None
    except (pd.errors.ParserError, pl.exceptions.ComputeError) as e:
        raise SystemExit(f"[오류] 데이터 로딩 실패: {e}") from e
...
def _na_impact(df):
    ...
    return {
        ...
        "pos_rate_raw": float(df[TARGET].eq(">50K").mean()),
        "pos_rate_kept": float(kept[TARGET].eq(">50K").mean()) if len(kept) else float("nan"),
    }


def clean(df, strategy=DROP):
