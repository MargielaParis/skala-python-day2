...
CORR_CMAP = LinearSegmentedColormap.from_list("corr", [
    "#104281", "#256abf", "#3987e5", "#9ec5f4", "#f0efec",
    "#f6c9c8", "#ee8b8a", "#e34948", "#b52c2b", "#8a1e1d"])
...
def _high_ratio(df, col):
    # 지정 컬럼별 고소득 비율과 표본 수
    return (df.assign(high=df["income"].eq(POSITIVE))
              .groupby(col)["high"].agg(ratio="mean", n="count"))
...
def save_seaborn_charts(df, file_path):
    ...
    ax = axes[0][0]
    sns.histplot(data=df, x="age", hue="income", hue_order=order, palette=palette,
                 bins=36, stat="density", common_norm=False, element="step",
    ...
    ax = axes[0][1]
    sns.boxplot(data=df, x="hours-per-week", y="income", order=order, hue="income",
                hue_order=order, palette=palette, legend=False, width=0.42,
    ...
    for i, grp in enumerate(order):                       # 평균만 선택적으로 직접 라벨
        ...
        ax.scatter(mean, i, s=46, color=SURFACE, zorder=3,
                   edgecolor=palette[grp], linewidth=2)
    ...
    ax.text(overall + 0.006, 1.005, f"전체 {overall:.1%}", fontsize=9, color=INK_SUB,
    ...
    hm = sns.heatmap(corr, mask=mask, cmap=CORR_CMAP, vmin=-1, vmax=1, annot=True,
                     fmt=".2f", annot_kws={"size": 10}, linewidths=2, linecolor=SURFACE,
                     cbar_kws={"shrink": 0.6, "pad": 0.03}, ax=ax)
    ...
    ax.tick_params(labelrotation=0), ax.set_xticklabels(ax.get_xticklabels(), rotation=20, ha="right")

    fig.tight_layout(pad=2.4)
    try:
        fig.savefig(file_path, dpi=150, bbox_inches="tight", facecolor=SURFACE)
    except OSError as e:
        raise SystemExit(f"[오류] 정적 차트 저장 실패: {e}")
    finally:
        plt.close(fig)
...
def save_plotly_chart(df, file_path):
    ...
    fig = go.Figure(go.Bar(
        x=ratio["ratio"], y=ratio.index, orientation="h", marker_color=LOW,
        marker_line_width=0, customdata=ratio["n"],
        text=[f"{v:.1%}" for v in ratio["ratio"]], textposition="outside",
        textfont={"size": 11, "color": INK_SUB},
        hovertemplate="<b>%{y}</b><br>>50K 비율 %{x:.1%}<br>표본 %{customdata:,}명<extra></extra>",
    ))
    fig.add_vline(x=overall, line_color=INK_MUTED, line_width=1,
                  annotation_text=f"전체 {overall:.1%}", annotation_position="top",
                  annotation_font={"size": 11, "color": INK_SUB})
    fig.update_layout(
        title={"text": "학력별 고소득(>50K) 비율<br>"
                       f"<span style='font-size:12px;color:{INK_MUTED}'>"
                       f"학력 연차 낮은 순 · 정제 후 {len(df):,}명</span>",
               "font": {"size": 18, "color": INK}, "x": 0.02},
        font={"family": "system-ui, -apple-system, 'Segoe UI', sans-serif", "color": INK_SUB},
        paper_bgcolor=SURFACE, plot_bgcolor=SURFACE, bargap=0.34,
        height=680, margin={"l": 150, "r": 60, "t": 96, "b": 56}, showlegend=False,
        hoverlabel={"bgcolor": "#ffffff", "bordercolor": AXIS, "font_size": 12},
    )
    fig.update_xaxes(title=">50K 비율", tickformat=".0%", gridcolor=GRID,
                     zeroline=False, linecolor=AXIS, ticks="",
    ...
    try:
        ...
        raise SystemExit(f"[오류] 인터랙티브 차트 저장 실패: {e}")
...
CAT_LAYOUT = [("education", "occupation"), ("native-country", "workclass"),
...
def _income_legend(fig, order, palette):
    ...
    handles = [Patch(facecolor=palette[g], alpha=0.30, edgecolor=palette[g],
                     linewidth=2, label=g) for g in order]
    fig.legend(handles=handles, loc="upper right", frameon=False,
               ncol=2, fontsize=11, labelcolor=INK_SUB,
...
def save_numeric_panels(df, file_path):
    ...
    for ax, col in zip(axes.ravel(), NUM_ALL):
        ...
        sns.histplot(data=data, x=col, hue="income", hue_order=order, palette=palette,
                     bins=36, discrete=discrete, stat="density", common_norm=False,
                     element="step", fill=True, alpha=0.30, linewidth=2,
                     log_scale=(skewed, False), ax=ax)
        note = (f"  (0이 아닌 {1 - zero_share:.1%}만 · 로그 축)" if skewed else
                "  (상위 0.5% 절단)" if clipped else "")
    ...
    fig.suptitle("수치형 변수 분포 — 소득 그룹별 (밀도 정규화)", fontsize=14, color=INK,
                 x=0.006, ha="left", y=0.985)
    fig.tight_layout(pad=1.4, rect=(0, 0, 1, 0.965))
    try:
        fig.savefig(file_path, dpi=150, bbox_inches="tight", facecolor=SURFACE)
    except OSError as e:
        raise SystemExit(f"[오류] 수치형 분포 차트 저장 실패: {e}")
    finally:
        plt.close(fig)
...
def save_categorical_panels(df, file_path):
    ...
    ratios = {c: _fold_rare(_high_ratio(df, c)).sort_values("ratio")
              for row in CAT_LAYOUT for c in row}
    heights = [max(len(ratios[a]), len(ratios[b])) for a, b in CAT_LAYOUT]
    overall = df["income"].eq(POSITIVE).mean()

    fig, axes = plt.subplots(len(CAT_LAYOUT), 2, figsize=(17, sum(heights) * 0.40 + 3),
                             height_ratios=heights, facecolor=SURFACE)
    for (left, right), row_axes, rows in zip(CAT_LAYOUT, axes, heights):
        for col, ax in zip((left, right), row_axes):
            r = ratios[col]
            labels = [f"{i} ({n:,})" for i, n in zip(r.index, r["n"])]
    ...
    fig.suptitle(f"범주형 변수별 고소득(>50K) 비율  ·  괄호는 표본 수  ·  세로선 = 전체 {overall:.1%}",
                 fontsize=14, color=INK, x=0.008, ha="left", y=0.997)
    fig.tight_layout(pad=2.0, rect=(0, 0, 1, 0.985))
    try:
        fig.savefig(file_path, dpi=150, bbox_inches="tight", facecolor=SURFACE)
    except OSError as e:
        raise SystemExit(f"[오류] 범주형 비율 차트 저장 실패: {e}")
    finally:
        plt.close(fig)
