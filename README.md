...
- LogisticRegression Pipeline: 정확도 0.8476 / 정밀도 0.7288 / 재현율 0.6049 / F1 0.6611 /
  ROC-AUC 0.9027 / PR-AUC 0.7651 (학습 adult.data 30,139건 / 평가 adult.test 15,055건, 정제 후 기준)
  - 실제 고소득 3,700명 중 1,462명을 저소득으로 놓쳤다 (혼동행렬은 `output/report.md` 4-1절)
  - 임계값 0.5는 자의적이다. **학습셋 5-폴드 교차검증의 out-of-fold 확률**에서 F1을 최대화하는
    0.304를 쓰면 재현율이 0.6049 → 0.7786, 놓친 고소득이 1,462명 → 819명이 된다 (4-2절)
...
- 결측 처리 방식 A/B: **평가셋을 15,055행으로 고정**하고 학습셋만 바꿔 비교한다.
  dropna F1 0.6611 vs Unknown 보존 F1 0.6603 — 격차 0.0008로 우열을 가릴 수 없다.
  다만 결측 행 삭제는 소득 그룹별 제거율이 달라(`<=50K` 8.36% vs `>50K` 4.25%) 표본 구성을 바꾼다.
- 민감도: `relationship`을 빼고 다시 학습하면 `sex_Female` 오즈비가 0.43 → **0.83**으로 바뀐다.
...
- `capital-gain`은 99,999로 상한 처리된 값이라 큰 계수를 금액 효과로 읽으면 안 된다.
  상한 도달 여부를 지시변수로 분리해봤지만 `capital-gain`과 상관이 0.94라 따로 식별되지 않아
  (계수 구간 [0.14, 7.14]) 모델에서 뺐다.
- 학습에 없던 범주는 전부 0으로 인코딩된다. 이번 실행에서 해당 평가 행은 0건이다(매번 계산해 싣는다).
